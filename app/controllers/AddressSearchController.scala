...
class AddressSearchController @Inject() (
  ...
  def searchByPostcode(): Action[LookupByPostcodeRequest] = accessCheckedAction(
    parse.json[LookupByPostcodeRequest]
  ) { implicit request: Request[LookupByPostcodeRequest] =>
    searchByPostcode(request)
  }

  def searchByUprn(): Action[LookupByUprnRequest] = accessCheckedAction(
    parse.json[LookupByUprnRequest]
  ) { implicit request: Request[LookupByUprnRequest] =>
    searchByUprn(request)
  }

  def searchByPostTown(): Action[LookupByPostTownRequest] = accessCheckedAction(
    parse.json[LookupByPostTownRequest]
  ...
  def searchByCountry(
      countryCode: String
  ): Action[LookupByCountryFilterRequest] = accessCheckedAction(
    parse.json[LookupByCountryFilterRequest]
  ...
  private[controllers] def searchByUprn(
      request: Request[LookupByUprnRequest]
  )(implicit
      hc: HeaderCarrier,
    ...
    forwardIfAllowed[LookupByUprnRequest, List[AddressRecord]](
      request,
      addresses =>
        auditor.auditAddressSearch(
          userAgent,
          addresses,
          uprn = Some(request.body.uprn)
        )
    )
  }

  private[controllers] def searchByPostcode[A](
      request: Request[LookupByPostcodeRequest]
  )(implicit
      hc: HeaderCarrier,
      userAgent: Option[UserAgent]
  ): Future[Result] = {
    ...
    forwardIfAllowed[LookupByPostcodeRequest, List[AddressRecord]](
      request,
      addresses =>
        auditor.auditAddressSearch(
          userAgent,
  ...
  private[controllers] def searchByTown[A](
      request: Request[LookupByPostTownRequest]
  )(implicit
      hc: HeaderCarrier,
      userAgent: Option[UserAgent]
  ): Future[Result] = {
    ...
    forwardIfAllowed[LookupByPostTownRequest, List[AddressRecord]](
      request,
      addresses =>
        auditor.auditAddressSearch(
          userAgent,
          addresses,
          posttown = Some(posttown.posttown.toUpperCase),
          filter = posttown.filter
        )
    )
  }

  private[controllers] def searchByCountry[A](
      request: Request[LookupByCountryRequest]
  )(implicit
      hc: HeaderCarrier,
      userAgent: Option[UserAgent]
  ): Future[Result] = {
    ...
    forwardIfAllowed[LookupByCountryRequest, List[NonUKAddress]](
      request,
      addresses =>
        auditor.auditNonUKAddressSearch(
          userAgent,
          country = country.country,
          filter = Option(country.filter),
          nonUKAddresses = addresses
        )
    )
  }

  private def addCountryTo(
      body: LookupByCountryFilterRequest,
      country: String
  ): LookupByCountryRequest = {
    LookupByCountryRequest(country, body.filter)
  }

  private def url(path: String) =
    s"${configHelper.addressSearchApiBaseUrl}$path"

  private def forwardIfAllowed[Req: Writes, Resp: Reads](
      request: Request[Req],
      auditFn: Resp => Unit
  ): Future[Result] = {
    val newHeadersMap = request.headers.toSimpleMap ++ Map(
  ...
  implicit def requestToHeaderCarrier[T](implicit
      request: Request[T]
  ): HeaderCarrier =
    HeaderCarrierConverter.fromRequest(request)

  implicit def requestToUserAgent[T](implicit
      request: Request[T]
  ): Option[UserAgent] =
    UserAgent(request)
