...
class Module(environment: Environment,
  ...
  @Provides
  @Singleton
  def provideTransactorOptional(configuration: Configuration,
  ...
  @Provides
  @Singleton
  def provideAddressSearcher(metrics: Metrics, configuration: Configuration, rdsQueryConfig: RdsQueryConfig,
                             executionContext: ExecutionContext, applicationLifecycle: ApplicationLifecycle): AddressSearcher = {

    val transactor = new TransactorProvider(configuration, applicationLifecycle).get(executionContext)
    val searcher = new AddressLookupRepository(transactor, rdsQueryConfig)
