...
class AddressTest extends AnyFunSuite with Matchers {
  ...
  test(
    ...
    val a = Address(List("ATown"), "some-town", "FX1 1XX", Option(England), GB)
    a.printable("\n") shouldBe "ATown\nsome-town\nFX1 1XX"
  }
  ...
  test(
    ...
    val a = Address(List("Line1", "Line2", "Line3"), "ATown", "FX1 1XX", Option(Wales), GB)
  ...
  test(
    ...
    val a = Address(List(
      ...
      "Llanfairpwllgwyngyllgogerychwyrndrobwllllantysiliogogogoch", "FX1 1XX", Option(England), GB).truncatedAddress()
    val expected = List(
  ...
  test(
    ...
    val a = Address(Nil, "ATown", "FX1 1XX", Option(Wales), GB)
    !a.isValid shouldBe true
  }

  test(
    """An address with four lines and a town is not valid""") {
    val a = Address(List("a", "b", "c", "d"), "ATown", "FX1 1XX", Option(Wales), GB)
    !a.isValid shouldBe true
  }

  test(
    """An address with five lines is not valid""") {
    val a = Address(List("a", "b", "c", "d", "e"), "some-town", "FX1 1XX", Option(Wales), GB)
    !a.isValid shouldBe true
  }
