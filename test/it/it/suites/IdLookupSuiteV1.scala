...
class IdLookupSuiteV1 @Inject() (val wsClient: WSClient, val appEndpoint: String)(implicit val app: Application)
  ...
  "id lookup" when {
    ...
    "successful" must {
      ...
      "set the cache-control header and include a positive max-age ignore it" ignore {
        val response = get("/uk/addresses/GB0001")
      ...
      "give a 404 response for an unknown id" in {
        val response = get("/uk/addresses/X0")
